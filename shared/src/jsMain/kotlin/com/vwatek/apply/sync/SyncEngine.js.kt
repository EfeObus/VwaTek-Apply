...
/**
 * Platform-specific ID generation for JS/Web
 */
actual fun generateSyncOperationId(): String {
    // Generate a UUID-like string using available JS APIs
    val timestamp = Date.now().toLong().toString(36)
    val randomPart = Random.nextLong().toString(36).takeLast(8)
    return "$timestamp-$randomPart"
}

/**
 * Platform-specific current time for JS/Web
 */
actual fun currentTimeMillis(): Long = Date.now().toLong()

/**
 * JS/Web factory for SyncEngine
 */
actual class SyncEngineFactory {
    ...
    actual fun create(
        ...
        val client = httpClient ?: throw IllegalStateException(
            ...
            "Call SyncEngineFactory().initialize(httpClient, networkMonitor) during app initialization."
        )
...
/**
 * JS/Web implementation of SyncEngine using localStorage
 */
internal class JsSyncEngine(
    ...
    private fun getStoredDeviceId(): String? {
        return try {
            localStorage.getItem("vwatek_device_id")
        } catch (e: Exception) {
            null
        }
    }
    ...
    private fun saveDeviceId(deviceId: String) {
        try {
            localStorage.setItem("vwatek_device_id", deviceId)
        } catch (e: Exception) {
            console.error("Failed to save device ID: ${e.message}")
        }
    }
    
    private fun getStoredLastSyncTimestamp(): Long {
        return try {
            localStorage.getItem("vwatek_last_sync_timestamp")?.toLongOrNull() ?: 0L
        } catch (e: Exception) {
            0L
        }
    }
    ...
    private fun saveLastSyncTimestamp(timestamp: Long) {
        try {
            localStorage.setItem("vwatek_last_sync_timestamp", timestamp.toString())
        } catch (e: Exception) {
            console.error("Failed to save last sync timestamp: ${e.message}")
        }
    }
}
